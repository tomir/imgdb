    ...
    <div class="container episode-list" id="content">
        ...
        <ul class="list-unstyled" id="episode-list" data-bind="moreLess, playVideos">
            @foreach($episodes as $episode)
                <li class="media">
                    @if ($episode->promo)
                        <a class="col-sm-3 play" href="{{ Helpers::episodeUrl($title->title, $title->id, $title->type, $num, $episode->episode_number) }}" data-source="{{ $episode->promo }}" data-poster="{{ str_replace('w300', 'original', $episode->poster) }}">
